...
(function () {
	...
	function draw() {
		...
		glow.addColorStop(0, 'rgba(7,190,184,0.16)');
		glow.addColorStop(1, 'rgba(7,190,184,0)');
		ctx.fillStyle = glow;
		...
		for (var b2 = 0; b2 < 4; b2++) {
			...
			ctx.fillStyle = 'rgba(107,233,245,' + alphas[b2] + ')';
			ctx.beginPath();
		...
		for (var a = 0; a < arcs.length; a++) {
			...
			for (var s = 0; s < steps; s++) {
				...
				ctx.strokeStyle = 'rgba(255,23,145,' + (0.85 * fade * depth) + ')';
				ctx.beginPath();
		...
		if (tp[2] > 0) {
			...
			if (landed > 0) {
				...
				ctx.strokeStyle = 'rgba(255,23,145,' + (0.5 * landed) + ')';
				ctx.lineWidth = 2;
				ctx.stroke();
			}
			ctx.beginPath();
